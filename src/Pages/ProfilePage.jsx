...
export default function SafeProfileMock() {
  ...
  const IconTip = ({ className = "w-5 h-5", active = false }) => (
    <svg className={`${className} ${active ? 'animate-glow text-blue-500' : ''}`} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" stroke="#9AA3AD">
  ...
  const toggleLike = async (id) => {
    ...
    try {
      if (typeof window !== "undefined" && window.localStorage) {
        localStorage.setItem("post_likes_state", JSON.stringify({ ...likedPosts, [postId]: !wasLiked }));
      }
    } catch (err) {
  ...
  const toggleTip = (id) => {
    ...
    setTimeout(() => setTipAnimatingPosts((prev) => ({ ...prev, [id]: false })), 1500);
  };
  ...
  useEffect(() => {
    ...
    <ErrorBoundary>
                ...
                {posts.map((p) => (
                  <div key={p.id} className="border-b pb-0 last:border-none px-4">
                    ...
                    <div className="flex items-center gap-6 text-gray-500 mb-2">
                      <button
                        onClick={isPostUnlocked(p.id) ? () => toggleLike(p.id) : () => openSubModal("monthly")}
                        className={isPostUnlocked(p.id) ? "" : "pointer-events-auto opacity-100"}
                      ...
                      <button
                        onClick={isPostUnlocked(p.id) ? () => {} : () => openSubModal("monthly")}
                        className={isPostUnlocked(p.id) ? "" : "pointer-events-auto opacity-100"}
                      ...
                      <button
                        onClick={isPostUnlocked(p.id) ? () => toggleTip(p.id) : () => openSubModal("monthly")}
                        className={isPostUnlocked(p.id) ? "flex items-center gap-1" : "flex items-center gap-1 pointer-events-auto opacity-100"}
                      ...
                      <button
                        onClick={isPostUnlocked(p.id) ? () => toggleBookmark(p.id) : () => openSubModal("monthly")}
                        className={isPostUnlocked(p.id) ? "ml-auto" : "ml-auto pointer-events-auto opacity-100"}
      ...
      <style>{`
        @keyframes glow {
          0% { box-shadow: 0 0 5px #0085C2; opacity: 1; }
          50% { box-shadow: 0 0 20px #0085C2; opacity: 0.8; }
          100% { box-shadow: 0 0 5px #0085C2; opacity: 1; }
        }
        .animate-glow {
