...
const DUMMY_POST_CAPTIONS = [
  ...
  "In the mood for mischief. Let's turn up the heat together. 🕵️‍♀️",
...
export default function SafeProfileMock() {
  ...
  useEffect(() => {
    ...
    try {
      if (typeof window !== "undefined" && window.localStorage) {
        ...
        if (likedState) {
          setPrivateLikes(JSON.parse(likedState));
        }
  ...
  const IconTip = ({ className = "w-5 h-5", active = false }) => (
    <span className={active ? 'animate-pulse-glow text-[#0085C2]' : ''}>
      <svg className={className} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" stroke={active ? "#0085C2" : "#9AA3AD"}>
        <circle cx="12" cy="12" r="9" strokeWidth="1.2"/><text x="12" y="15.2" textAnchor="middle" fontSize="10" fill={active ? "#0085C2" : "#9AA3AD"} fontWeight="600">$</text>
      </svg>
    </span>
  );
  ...
  const toggleLike = (id) => {
    ...
    const wasLiked = !!privateLikes[postId];
    setPrivateLikes((prev) => ({ ...prev, [postId]: !wasLiked }));
  };
  ...
  const sendMessage = async (message) => {
    ...
    try {
      const { error } = await supabase
        .from("messages")
        .insert([{
          creator_handle: creator.handle.replace("@", ""),
    ...
    <ErrorBoundary>
            ...
            <div className="flex gap-2">
              ...
              >
                <svg className="w-4 h-4 text-[#06b6d4]" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M12 17.3l6.18 3.9-1.64-7.03L21 9.24l-7.19-.62L12 2 10.19 8.62 3 9.24l4.46 4.93L5.82 21.2z" stroke="#06b6d4" strokeWidth="0.8" fill="none" />
                </svg>
              </button>
                ...
                {posts.map((p) => (
                  <div key={p.id} className="border-b pb-0 last:border-none px-4">
                    ...
                    <div className="relative w-full mb-2">
                      {isPostUnlocked(p.id) ? (
                          ...
                          {p.mediaType === "video" ? (
                            ...
                            >
                              <video
                                ...
                                preload="metadata"
                                poster={videoThumbnails[p.id] || p.mediaSrc + '#t=1'}
                              />
                        ...
                        >
                          ...
                          {/* Functional Button Overlay - EXACT position from screenshot calculation */}
                          <button
                    ...
                    <div className="flex items-center gap-6 text-gray-500 mb-2">
                      ...
                      >
                        <IconLike active={!!privateLikes[p.id]} />
                      </button>
                      ...
                      <button
                        ...
                        className={isPostUnlocked(p.id) ? "flex items-center gap-1" : "pointer-events-none opacity-50 flex items-center gap-1"}
                        aria-label={`tip post ${p.id}`}
                      >
                        <IconTip active={!!tipAnimatingPosts[p.id]} />
                        <span className={!!tipAnimatingPosts[p.id] ? 'animate-pulse-glow text-[#0085C2]' : 'text-[13px]'}>SEND TIP</span>
                      </button>
      ...
      <style>{`
        @keyframes pulse-glow {
          ...
          50% { fill-opacity: 0.5; stroke-opacity: 0.5; }
