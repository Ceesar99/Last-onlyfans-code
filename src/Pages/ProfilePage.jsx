...
function buildLocalDummyPosts() {
  ...
  return Array.from({ length: 100 }).map((_, i) => {
    ...
    return {
      ...
      locked: true, // ALL locked by default - old way
      isDummy: true,
    };
  });
}
...
export default function SafeProfileMock() {
  ...
  const [messagesUnlocked, setMessagesUnlocked] = useState(false); // starts false
  const toastTimerRef = useRef(null);

  const [showSubModal, setShowSubModal] = useState(false);
  const [showAddCard, setShowAddCard] = useState(false); // restored
  ...
  const [userEmail, setUserEmail] = useState(""); // only set from AddCardForm

  const processedVideoIds = useRef(new Set());
  ...
  const logPaymentAttempt = async (payload) => {
    try {
      ...
      await supabase.from("payment_logs").insert([record]);
    } catch (err) {}
  };

  // Full handleAddCardSubmit restored
  const handleAddCardSubmit = (payload) => {
    ...
    if (!unlockedOnceRef.current) {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => showToast("you can now message creator directly🤤💦"), 2000);
  ...
  const openAddCard = () => { setShowAddCard(true); lockScroll(); };
  ...
  useEffect(() => {
    ...
    // Load userEmail ONLY if it exists in LS (set by AddCardForm)
    if (typeof window !== "undefined" && window.localStorage) {
    ...
    // Load liked posts with correct key
    try {
      if (typeof window !== "undefined" && window.localStorage) {
  ...
  // Video thumbnails - only once
  useEffect(() => {
    const generateThumbnail = async (videoSrc, postId) => {
      if (processedVideoIds.current.has(postId)) return;
      processedVideoIds.current.add(postId);
      ...
      return new Promise((resolve) => {
        ...
        video.addEventListener('seeked', () => {
          ...
          setVideoThumbnails(prev => ({ ...prev, [postId]: canvas.toDataURL('image/jpeg') }));
          resolve();
        }, { once: true });
        video.load();
      });
    ...
    posts.forEach(post => {
      if (post.mediaType === 'video' && post.mediaSrc && !processedVideoIds.current.has(post.id)) {
        generateThumbnail(post.mediaSrc, post.id);
      }
    });
  }, [posts]);
  ...
  useEffect(() => {
    ...
    try {
      if (typeof window !== "undefined" && window.localStorage) {
        ...
        if (localStorage.getItem("messages_unlocked") === "true") {
          setMessagesUnlocked(true);
        }
      }
    } catch (e) {
  ...
  const IconTip = ({ className = "w-5 h-5", active = false }) => (
    <span className={active ? 'animate-breathe' : ''}>
      <svg className={className} viewBox="0 0 24 24" fill={active ? "#0085C2" : "none"} xmlns="http://www.w3.org/2000/svg" aria-hidden="true" stroke={active ? "#0085C2" : "#9AA3AD"}>
        <circle cx="12" cy="12" r="9" strokeWidth="1.2" />
        <text x="12" y="15.2" textAnchor="middle" fontSize="10" fill={active ? "#0085C2" : "#9AA3AD"} fontWeight="600">$</text>
      </svg>
    </span>
  );
        ...
        >
          ...
          <ModalPortal isOpen={showSubModal} onClose={closeSubModal} zIndex={1000}>
            <SubscriptionModal
              ...
              onAddCard={openAddCard} // added back
              onClose={closeSubModal}
              freeSampleActive={freeSample.active}
            />
          </ModalPortal>

          {/* ADD CARD MODAL RESTORED */}
