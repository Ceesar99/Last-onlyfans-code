// ProfilePage.jsx - FIXED VERSION (BUNDLE 1 OF 2)
...
export default function SafeProfileMock() {
      ...
      try {
        ...
        if (handle) {
          try {
            ...
            // Continue with default creator data
          }
        }

        // Posts fetch with proper error handling
        try {
          ...
          if (postsError) {
            ...
            // Keep dummy posts, just log the error
  ...
  const toggleLike = async (id) => {
    const userEmail = typeof window !== "undefined" ? window.localStorage.getItem("user_email") : null;
    if (!userEmail) {
      showToast("Please subscribe to like posts", "error");
      return;
    }
  ...
  return (
    <ErrorBoundary>
      <div className="min-h-screen bg-gray-100 flex justify-center p-4">
        <div className="w-full max-w-xl bg-white rounded-md shadow-sm text-[15px] relative profile-card" style={{ maxHeight: "calc(100vh - 2rem)", overflowY: "auto" }}>
          ...
          <div className="px-4 mt-2 flex justify-end">
            <div className="flex gap-2">
              <button onClick={() => { setStarred(!starred); showToast(starred ? "Unstarred" : "Starred"); }} className="w-9 h-9 bg-white rounded-full border flex items-center justify-center shadow text-[#06b6d4]" aria-label="star profile">
              ...
              <button onClick={async () => { const href = typeof window !== "undefined" && window.location ? window.location.href : "https://example.com/profile"; if (navigator.share) { try { await navigator.share({ title: document.title || "Profile", url: href }); showToast("Shared!"); return; } catch (err) {} } try { await navigator.clipboard.writeText(href); showToast("Link copied!"); } catch { showToast("Could not copy link — copy manually", "error"); } }} className="w-9 h-9 bg-white rounded-full border flex items-center justify-center shadow text-[#06b6d4]" aria-label="share profile">
          ...
          <div className="px-4 mt-2 flex items-center justify-between">
            ...
            <button onClick={openMessageModal} className="bg-[#00AFF0] text-white text-sm font-semibold rounded-full px-6 py-2 shadow min-w-[120px]" aria-label="message creator">Message</button>
          </div>
          ...
          <div className="px-4 border-t mt-3 pt-3">
            ...
            <button onClick={() => setBioExpanded(!bioExpanded)} className="text-[13px] text-blue-500 underline mt-2 inline-block" aria-expanded={bioExpanded}>{bioExpanded ? "Collapse" : "More info"}</button>
          </div>
          ...
          <div className="mt-4 border-t">
            <div className="grid grid-cols-2 bg-white text-[14px] font-medium">
              <button onClick={() => setActiveTab("posts")} className={`py-3 ${activeTab === "posts" ? "border-b-2 border-black" : "text-gray-500"}`} aria-pressed={activeTab === "posts"}>{(2918 + posts.length).toLocaleString()} POSTS</button>
              <button onClick={() => setActiveTab("media")} className={`py-3 ${activeTab === "media" ? "border-b-2 border-black" : "text-gray-500"}`} aria-pressed={activeTab === "media"}>{(1939 + mediaItems.length).toLocaleString()} MEDIA</button>
            </div>
          </div>
                        ...
                        <div className="mt-3">
                          {p.mediaType && !isPostUnlocked(p.id) ? (
                              ...
                              <div className="flex flex-col items-center">
                                ...
                                <div className="w-full mt-3 max-w-[420px]">
                                  <div className="border rounded-md p-3 bg-white">
                                    ...
                                    <div className="mt-3">
                                      <button onClick={() => openSubscriptionModalWithPlan("monthly")} className="mx-auto block px-6 py-2 rounded-full bg-[#00AFF0] text-white font-semibold text-sm max-w-[300px]">SUBSCRIBE TO SEE USER'S POSTS</button>
                                    </div>
                                  </div>
                                </div>
                            ...
                            <div className="rounded-md overflow-hidden bg-gray-100 h-44 flex items-center justify-center relative">
                              {p.mediaType === "video" ? (
                                <div role="button" tabIndex={0} onClick={() => openViewer({ list: buildViewerListFromPosts, index: buildViewerListFromPosts.findIndex((x) => x.id === p.id) })} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { openViewer({ list: buildViewerListFromPosts, index: buildViewerListFromPosts.findIndex((x) => x.id === p.id) }); } }} className="w-full h-full cursor-pointer flex items-center justify-center" aria-label={`Open video post ${p.id}`}>
                                ...
                                <img src={p.mediaSrc} alt={`post media ${p.id}`} className="w-full h-full object-cover cursor-pointer" loading="lazy" onClick={() => openViewer({ list: buildViewerListFromPosts, index: buildViewerListFromPosts.findIndex((x) => x.id === p.id) })} />
                              )}
                            </div>
                          ) : null}
                        </div>
                    ...
                    return (
                      <div key={m.id} className="relative bg-white aspect-square border border-transparent">
                        {locked ? (
                          ...
                          <>
                            {m.type === "image" ? (
                              <img src={m.src} alt={`media ${m.id}`} className="w-full h-full object-cover cursor-pointer" loading="lazy" onClick={() => openViewer({ list: buildViewerListFromMedia, index: buildViewerListFromMedia.findIndex((x) => x.id === m.id) })} />
                            ) : (
                              <div className="w-full h-full bg-black flex items-center justify-center text-white text-xs cursor-pointer" onClick={() => openViewer({ list: buildViewerListFromMedia, index: buildViewerListFromMedia.findIndex((x) => x.id === m.id) })} role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { openViewer({ list: buildViewerListFromMedia, index: buildViewerListFromMedia.findIndex((x) => x.id === m.id) }); } }}>VIDEO</div>
                            )}
                            {m.type === "video" && (
                ...
                {!subscribed && (
                  <div className="mt-4 px-2">
                    <div className="border rounded p-3 bg-white">
                      <button onClick={() => openSubscriptionModalWithPlan("monthly")} className="w-full rounded-full py-3 font-semibold text-white bg-[#00AFF0]">SUBSCRIBE TO SEE USER'S MEDIA</button>
                    </div>
                  </div>
                )}
              </div>
          ...
          <ModalPortal isOpen={showSubModal} onClose={closeSubModal} zIndex={1000}>
            <SubscriptionModal creator={creator} selectedPlan={selectedPlan} onSelectPlan={(planId) => setSelectedPlan(planId)} onAddCard={(planId, stage) => { setSelectedPlan(planId || selectedPlan || "monthly"); openAddCard(planId, stage); }} onClose={closeSubModal} freeSampleActive={freeSample.active} />
          </ModalPortal>
