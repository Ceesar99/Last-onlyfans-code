...
function buildLocalDummyPosts() {
  ...
  return Array.from({ length: 100 }).map((_, i) => {
    ...
    return {
      ...
      mediaType: "image",
      mediaSrc: hasRealImage ? UNLOCKED_POST_IMAGES[i] : "https://via.placeholder.com/600x800//666666?text=Locked+Content",
...
export default function SafeProfileMock() {
  ...
  // -----------------------
  // SUBSCRIBE FLOW HANDLER (renamed from handleAddCardSubmit, removed logs, added simulated failure)
  // -----------------------
  const handleSubscribe = () => {
    // Simulate payment failure (20% chance) - on failure, do nothing (leave locked)
    if (Math.random() < 0.2) {
      return; // Silent failure as per removed error messages
    }
