// ProfilePage.jsx - COMPLETE WITH APPLOADMANAGER
...
function getUserLikesKey(userEmail) {
  if (!userEmail) return "post_likes_state";
  return `post_likes_state_${userEmail}`;
}
...
export default function SafeProfileMock() {
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined" && window.localStorage) {
      ...
      if (storedEmail) {
        ...
        currentUserEmail = "subscriber@example.com";
    ...
    try {
      if (typeof window !== "undefined" && window.localStorage) {
        const likesKey = getUserLikesKey(currentUserEmail);
        const likedState = localStorage.getItem(likesKey);
        if (likedState) {
      ...
      console.warn("failed to read liked posts state", e);
    }
    ...
    const loadInitialData = async () => {
      ...
      try {
        if (handle) {
          ...
          if (profileError) {
            ...
            if (profileData.avatar_url) {
              const tId = `profile:avatar`;
              safeAddTask(tId);
              preloadImage(profileData.avatar_url).then(() => safeCompleteTask(tId));
            }

            // Preload banner
            if (profileData.banner_url) {
              const tId = `profile:banner`;
              safeAddTask(tId);
              preloadImage(profileData.banner_url).then(() => safeCompleteTask(tId));
            }
        ...
        if (postsError) {
          ...
          // Preload post images
          safeAddTask("profile:posts");
          const mediaPreloads = [];
          mappedDB.forEach((p) => {
            if (p.mediaSrc && p.mediaType === "image") {
              const id = `post:${p.id}:media`;
              safeAddTask(id);
              const pr = preloadImage(p.mediaSrc).then(() => safeCompleteTask(id));
              mediaPreloads.push(pr);
            }
          });

          Promise.all(mediaPreloads).then(() => {
            safeCompleteTask("profile:posts");
          });
    ...
    const postsChannel = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'posts' }, (payload) => {
        ...
        const newLikeCount = getStableLikeCount(postId);
  ...
  useEffect(() => {
    ...
    const generateThumbnails = async () => {
      ...
      for (const post of posts) {
        if (post.mediaType === 'video' && post.mediaSrc && !videoThumbnails[post.id]) {
          thumbs[post.id] = await generateThumbnail(post.mediaSrc);
        }
      }
  ...
  useEffect(() => {
    const map = {};
    posts.forEach((p) => {
      if (likeCounts[p.id] === undefined) {
        map[p.id] = p.likes;
      }
    });
    
    if (Object.keys(map).length > 0) {
      setLikeCounts((prev) => ({ ...prev, ...map }));
    }
  ...
  const IconTip = ({ className = "w-5 h-5", active = false }) => (
    <span className={active ? 'animate-breathe' : ''}>
      <svg className={className} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" stroke="#9AA3AD">
  ...
  const LockSVGStatic = () => (
    <div style={{ width: "100%", aspectRatio: "1614 / 843", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 1614 843" preserveAspectRatio="xMidYMid meet" style={{ width: "100%", height: "100%", objectFit: "contain" }}>
        <g id="dd4789b5-03b7-41b2-a148-534740596f4f">
          <rect style={{ stroke: "rgb(193,193,193)", 
                        strokeWidth: 0, 
  ...
  const toggleLike = (id) => {
    ...
    if (post && post.locked && !subscribed && !freeSample.active) {
      return;
    }
    
    const wasLiked = !!likedPosts[postId];
    const newLikedState = { ...likedPosts, [postId]: !wasLiked };
    ...
    try {
      if (typeof window !== "undefined" && window.localStorage) {
        const likesKey = getUserLikesKey(userEmail);
        localStorage.setItem(likesKey, JSON.stringify(newLikedState));
      }
    } catch (err) {
  ...
  const toggleTip = (id) => {
    ...
    setTimeout(() => setTipAnimatingPosts((prev) => ({ ...prev, [id]: false })), 2500);
  };
      ...
      <div className="min-h-screen bg-gray-100">
        <div className="w-full min-h-screen bg-white text-[15px] relative overflow-y-auto" style={{ zoom: "100%" }}>
          ...
          <div className="relative h-36 bg-gray-200 overflow-hidden">
            <img src={creator.banner || "https://via.placeholder.com/1200x400"} alt="banner" className="w-full h-full object-cover" />
          ...
          <div className="px-4 -mt-10 flex items-start">
            <div className="relative">
              <div className="w-20 h-20 rounded-full overflow-hidden shadow-md">
                <img src={creator.avatar || "https://via.placeholder.com/200"} alt="avatar" className="w-full h-full object-cover" />
              </div>
          ...
          <div className="px-4 -mt-2 flex justify-end">
            <div className="flex gap-2">
              <button onClick={() => { setStarred(!starred); }} className="w-9 h-9 bg-white rounded-full border flex items-center justify-center shadow text-[#06b6d4]" aria-label="star profile">
          ...
          <div className="px-4 mt-1 flex items-center justify-between gap-3">
            ...
            <button onClick={openMessageModal} className="bg-[#00AFF0] text-white text-sm font-semibold rounded-full px-6 py-2 shadow flex-shrink-0" aria-label="message creator">
              Message
            </button>
          </div>
              ...
              {posts.map((p) => (
                <div key={p.id} className="border-b pb-0 last:border-none px-4">
                  ...
                  <div className="relative w-full mb-2">
                    {isPostUnlocked(p.id) ? (
                      <div className="rounded-lg overflow-hidden bg-gray-100" style={{ width: "100%", aspectRatio: "1614 / 843" }}>
                        {p.mediaType === "video" ? (
                          <div
                            role="button"
                            tabIndex={0}
                          ...
                          >
                            <video src={p.mediaSrc} className="w-full h-full object-cover" preload="metadata" />
                      ...
                      <div className="relative bg-[#F8FAFB] rounded-lg overflow-hidden" style={{ width: "100%", aspectRatio: "1614 / 843" }}>
                        ...
                        <button
                          ...
                          style={{ width: "calc(100% - 40px)", height: "40px", bottom: "16px", left: "20px", padding: "0 16px", zIndex: 50 }}
                        >
                  ...
                  <div className="flex items-center gap-6 text-gray-500 mb-2">
                    <button onClick={isPostUnlocked(p.id) ? () => toggleLike(p.id) : () => {}} className={isPostUnlocked(p.id) ? "" : "pointer-events-none opacity-50"} aria-label={`like post ${p.id}`}>
                      <IconLike active={!!likedPosts[p.id]} />
                    </button>

                    <button onClick={isPostUnlocked(p.id) ? () => {} : () => {}} className={isPostUnlocked(p.id) ? "" : "pointer-events-none opacity-50"} aria-label={`comment on post ${p.id}`}>
                      <IconComment />
                    </button>

                    <button onClick={isPostUnlocked(p.id) ? () => toggleTip(p.id) : () => {}} className={isPostUnlocked(p.id) ? "flex items-center gap-1" : "flex items-center gap-1 pointer-events-none opacity-50"} aria-label={`tip post ${p.id}`}>
                    ...
                    <button onClick={isPostUnlocked(p.id) ? () => toggleBookmark(p.id) : () => {}} className={isPostUnlocked(p.id) ? "ml-auto" : "ml-auto pointer-events-none opacity-50"} aria-label={`bookmark post ${p.id}`}>
                          ...
                          {m.type === "image" ? (
                            ...
                            >
                              {videoThumbnails[m.id] ? (
                                <img src={videoThumbnails[m.id]} alt={`thumbnail for media ${m.id}`} className="w-full h-full object-cover" />
                              ) : (
                                <div className="bg-black w-full h-full flex items-center justify-center text-white text-xs">VIDEO</div>
                              )}
                              <div className="absolute inset-0 flex items-center justify-center">
                                <div className="bg-black bg-opacity-40 rounded-full p-3">
                                  <svg viewBox="0 0 24 24" className="w-8 h-8 text-white" fill="white" xmlns="http://www.w3.org/2000/svg">
