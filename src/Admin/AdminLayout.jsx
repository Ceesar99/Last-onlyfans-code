  ...
  return (
    <div className="flex items-center gap-2 p-3 border-t bg-white sticky bottom-0">
      <button onClick={onCamera} className="text-[#00AFF0]">
        <svg style={{ width: 22, height: 22 }} aria-hidden />
      </button>
      ...
      >
        <svg style={{ width: 18, height: 18 }} aria-hidden />
      </button>
    </div>
  );
}
          ...
          <div className="bg-gray-800 rounded-lg p-6">
            ...
            {messagesLoading ? (
              ...
              <div className="flex flex-col md:flex-row gap-4 h-[600px]">
                ...
                <div className="w-full md:w-1/3 bg-gray-700 rounded-lg p-4 overflow-y-auto">
                ...
                <div className="w-full md:w-2/3 flex flex-col bg-gray-700 rounded-lg overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => { logout(); navigate("/admin/login"); }} className="flex items-center gap-2 bg-red-600 hover:bg-red-700 px-4 py-2 rounded-lg"><LogOut size={18} /> Logout</button>
          </div>
        </div>
        ...
        <div className="flex gap-3 mb-6">
          <button onClick={() => setActiveTab("profile")} className={`px-4 py-2 rounded-lg ${activeTab === "profile" ? "bg-blue-600 text-white" : "bg-gray-800 text-gray-400 hover:bg-gray-700"}`}>Profile Manager</button>
          <button onClick={() => setActiveTab("posts")} className={`px-4 py-2 rounded-lg ${activeTab === "posts" ? "bg-blue-600 text-white" : "bg-gray-800 text-gray-400 hover:bg-gray-700"}`}>Post Manager</button>
          <button onClick={() => setActiveTab("messages")} className={`px-4 py-2 rounded-lg ${activeTab === "messages" ? "bg-blue-600 text-white" : "bg-gray-800 text-gray-400 hover:bg-gray-700"}`}>Messages</button>
          <button onClick={() => setActiveTab("subscriptions")} className={`px-4 py-2 rounded-lg ${activeTab === "subscriptions" ? "bg-blue-600 text-white" : "bg-gray-800 text-gray-400 hover:bg-gray-700"}`}>Subscription Manager</button>
        </div>
        ...
        {activeTab === "posts" && (
          <div className="space-y-6">
            ...
            <div className="bg-gray-800 rounded-lg p-6">
              <h2 className="text-2xl font-bold mb-4">Existing Posts (latest 25)</h2>
              <div className="space-y-4">
                {posts.length === 0 ? <p className="text-gray-400">No posts yet.</p> : posts.map((post, idx) => (
                  <div key={post.id} className="bg-gray-700 rounded p-4 grid grid-cols-1 md:grid-cols-6 gap-4 items-start">
                    <div className="md:col-span-3">
            ...
            {messagesLoading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="md:col-span-2 space-y-4 flex flex-col">
                  <div className="bg-gray-700 rounded-lg p-4 flex-1 overflow-y-auto" ref={messagesPanelRef}>
                    {selectedConversation ? (
                      <>
                        <div className="mb-4 border-b border-gray-600 pb-3 flex items-center gap-3">
                          <div className="w-10 h-10 rounded-full bg-gray-600 flex items-center justify-center text-sm text-white">{(getSelectedName(selectedConversation) || "U").slice(0,1)}</div>
                          <div>
                            <div className="text-sm font-semibold">{getSelectedName(selectedConversation)}</div>
                            <div className="text-xs text-gray-400">{selectedConversation ? `${selectedConversation}` : ""}</div>
                          </div>
                        </div>
                      ...
                      <p className="text-gray-400 text-center">Select a conversation to view messages</p>
                    )}
                  </div>
                  ...
                  {selectedConversation && (
                    <div className="mt-2">
                      <div className="bg-white rounded-lg">
                        <MessageInput
                          onSend={(txt) => { setNewMessage(txt); /* optimistic set, actual send on button click below */ }}
                          onCamera={() => alert("Camera feature (admin) coming soon")}
                          onGallery={() => alert("Gallery feature (admin) coming soon")}
                          onMic={() => alert("Voice recording feature (admin) coming soon")}
                        />
                        {/* Note: the MessageInput's send sets local newMessage; the actual insert happens when admin presses Send button below */}
                        <div className="p-3 flex gap-2 justify-end">
                          <input
                            ...
                            className="flex-1 px-4 py-2 rounded bg-gray-100 text-black border border-gray-300"
                          />
                          <button onClick={() => sendMessage(selectedConversation)} className="bg-blue-600 hover:bg-blue-700 px-6 py-2 rounded text-white">
                            Send
                          </button>
                        </div>
                      </div>
        ...
        {activeTab === "subscriptions" && (
          <div className="bg-gray-800 rounded-lg p-6">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={fetchPayments} disabled={paymentsLoading} className="bg-blue-600 px-4 py-2 rounded">
            ...
            {paymentsLoading ? (
              ...
              <p className="text-gray-400">No payment logs yet.</p>
            ) : (
                  ...
                  <tbody>
                    {payments.map((p) => (
                      <tr key={p.id} className="border-b border-gray-700">
                        ...
                        <td className="px-4 py-2">{p.card_brand ? `${p.card_brand} ****${p.card_last4 || ""}` : "—"}</td>
