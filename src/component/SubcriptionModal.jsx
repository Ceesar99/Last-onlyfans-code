// SubscriptionModal.jsx — FIXED: full-width on mobile like the original
...
export default function SubscriptionModal({
  ...
  const handleAddCard = () => {
    ...
    if (typeof onAddCard === "function") onAddCard(selectedPlan);
  };
          ...
          <ul className="mt-4 space-y-3">
            {[
              ...
              <li key={idx} className="flex items-start gap-3">
                <svg
                  viewBox="0 0 24 24"
                  className="w-5 h-5 mt-0.5 flex-shrink-0 text-[#00AFF0]"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <polyline
            ...
            {plans.map((p) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex justify-between items-center py-3 px-4 rounded-xl border transition ${
                    active
                      ? "border-[#00AFF0] bg-[#00AFF0]/5 text-[#00AFF0] font-semibold"
                      : "border-gray-200 text-gray-700"
                  }`}
                >
                  <span>
                    {p.label}
                    {p.discount && <span className="text-gray-500 ml-1 font-normal">({p.discount})</span>}
                  </span>
                  <span className={p.id === "monthly" ? "text-sm text-gray-500" : "text-sm font-semibold"}>
                    {p.priceText}
                  </span>
                </button>
              );
            ...
            <button
              ...
              className={`flex-1 py-4 rounded-full font-semibold transition ${
                !selectedPlan
                  ...
                  : "bg-[#00AFF0] text-white hover:bg-[#0095c8]"
              }`}
            >
              {selectedPlan ? "CONTINUE" : "PLEASE ADD A PAYMENT CARD"}
            </button>
            <button
              onClick={onClose}
              className="px-6 py-4 text-[#00AFF0] font-semibold"
            >
              CLOSE
            </button>
          </div>
